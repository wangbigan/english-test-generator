import { generateText } from "ai"
import { buildSamplePaper } from "./build-sample-paper"
import { getFinalPromptTemplate } from "../components/prompt-config-dialog"
import { createLanguageModel, getModelGenerationParams } from "@/lib/ai-provider"
import { testPaperSchema } from "@/lib/test-schema"
import type { TestConfig, AIProviderConfig, PromptConfig, GenerateTestResult } from "@/lib/types"

export async function generateTestPaper(
  config: TestConfig,
  openaiConfig: AIProviderConfig,
  promptConfig?: PromptConfig,
): Promise<GenerateTestResult> {
  const { systemMessage, userMessage } = buildMessages(config, promptConfig)
  const prompt = `${systemMessage}\n\n${userMessage}`

  // 没有 API Key 时直接返回示例试卷
  if (!openaiConfig?.apiKey?.trim()) {
    console.warn("API Key missing – falling back to local sample paper")
    return { test: buildSamplePaper(config), prompt, rawResponse: undefined }
  }

  try {
    const model = createLanguageModel(openaiConfig)
    const extraParams = getModelGenerationParams(openaiConfig)

    console.log("[API Call] Model:", openaiConfig.model)
    console.log("[API Call] BaseURL:", openaiConfig.baseUrl)

    const { text } = await generateText({
      model,
      messages: [
        { role: "system", content: systemMessage },
        { role: "user", content: userMessage },
      ],
      temperature: 0.7,
      ...extraParams,
    } as Parameters<typeof generateText>[0])

    const content = text
    console.log("[API Response] Success, content length:", content?.length || 0)

    if (!content) {
      throw new Error("No content received from API.")
    }

    // 去除 ```json ... ``` 代码块包裹
    const cleanedText = content
      .replace(/```json/gi, "")
      .replace(/```/g, "")
      .trim()

    console.log("[AI Raw Response]", content)
    console.log("[AI Cleaned Text]", cleanedText)

    // 防御性 JSON 解析
    const parsed = safeJsonParse(cleanedText)

    // Zod Schema 验证
    const validated = testPaperSchema.parse(parsed)

    return { test: validated, prompt, rawResponse: cleanedText }
  } catch (error) {
    console.error("Error generating test paper:", error)

    // 网络错误特殊提示
    if (error instanceof Error) {
      if (error.message?.includes("Failed to fetch") || error.name === "TypeError") {
        console.error("Network error detected. Possible causes:")
        console.error("1. Incorrect Base URL for the model")
        console.error("2. API endpoint not accessible")
        console.error("3. CORS issues")
        console.error("4. Invalid API key")

        if (openaiConfig.model.startsWith("kimi") || openaiConfig.model.startsWith("moonshot")) {
          console.error("For Kimi models, ensure Base URL is: https://api.moonshot.cn/v1")
        } else if (openaiConfig.model.startsWith("Doubao")) {
          console.error("For Doubao models, ensure Base URL is: https://ark.cn-beijing.volces.com/api/v3/chat/completions")
        }
      }

      // Zod 验证失败也打印具体错误
      if (error.name === "ZodError") {
        console.error("AI response failed schema validation:", error.message)
      }
    }

    // API 调用失败时返回示例试卷，避免 UI 崩溃
    console.warn("API call failed, falling back to sample paper")
    return { test: buildSamplePaper(config), prompt, rawResponse: undefined }
  }
}

export function buildMessages(
  config: TestConfig,
  promptConfig?: PromptConfig,
): { systemMessage: string; userMessage: string } {
  // System 消息：JSON 格式限定和风险控制
  const systemMessage = `你是一名资深的小学英语老师，擅长按照配置和要求生成高质量的英语试卷或题目。请严格遵循以下要求：

## 输出格式要求
- 必须严格按照JSON示例格式输出，不要包含任何其他内容
- 务必确保在token长度限制内输出完整JSON结构，且语法正确。
- 在满足上述要求的前提下，尽可能保证生成的题目数量最大化满足各题型要求，如实在不能满足，在json的questionNumber字段附加注视说明

## 内容要求：
- 题目设计必须确保正确答案只有一个，且不能出现二义性
- 材料、题干和选项中涉及数字部分都尽可能用英文单词，避免用数字表示

## 内容安全要求
- 生成的内容必须适合小学生，积极健康
- 不得包含任何政治敏感、宗教争议、暴力、色情等不当内容
- 避免涉及种族、性别、地域等歧视性内容
- 确保所有题目内容符合教育规范和社会主义核心价值观
`

  // 获取 prompt 模板（包含 JSON 示例）
  const template = getFinalPromptTemplate(promptConfig || null)

  // 准备变量替换的数据
  const variables: Record<string, string> = {
    grade: getGradeName(config.grade),
    difficulty: getDifficultyName(config.difficulty),
    theme: config.theme,
    knowledgePoints: config.knowledgePoints,
    totalScore: config.totalScore.toString(),
    totalQuestions: String(
      config.questionTypes.listening.count +
      config.questionTypes.multipleChoice.count +
      config.questionTypes.fillInBlank.count +
      config.questionTypes.reading.count +
      config.questionTypes.writing.count +
      config.questionTypes.trueFalse.count,
    ),

    // 听力题
    listeningCount: String(config.questionTypes.listening.count),
    listeningScore: String(config.questionTypes.listening.score),
    listeningTotalScore: String(config.questionTypes.listening.count * config.questionTypes.listening.score),

    // 选择题
    multipleChoiceCount: String(config.questionTypes.multipleChoice.count),
    multipleChoiceScore: String(config.questionTypes.multipleChoice.score),
    multipleChoiceTotalScore: String(
      config.questionTypes.multipleChoice.count * config.questionTypes.multipleChoice.score,
    ),

    // 填空题
    fillInBlankCount: String(config.questionTypes.fillInBlank.count),
    fillInBlankScore: String(config.questionTypes.fillInBlank.score),
    fillInBlankTotalScore: String(
      config.questionTypes.fillInBlank.count * config.questionTypes.fillInBlank.score,
    ),

    // 阅读题
    readingCount: String(config.questionTypes.reading.count),
    readingScore: String(config.questionTypes.reading.score),
    readingTotalScore: String(config.questionTypes.reading.count * config.questionTypes.reading.score),

    // 写作题
    writingCount: String(config.questionTypes.writing.count),
    writingScore: String(config.questionTypes.writing.score),
    writingTotalScore: String(config.questionTypes.writing.count * config.questionTypes.writing.score),

    // 判断题
    trueFalseCount: String(config.questionTypes.trueFalse.count),
    trueFalseScore: String(config.questionTypes.trueFalse.score),
    trueFalseTotalScore: String(config.questionTypes.trueFalse.count * config.questionTypes.trueFalse.score),
  }

  // 替换 Prompt 模板中的变量
  let userMessage = template
  Object.entries(variables).forEach(([key, value]) => {
    const regex = new RegExp(`{{${key}}}`, "g")
    userMessage = userMessage.replace(regex, value)
  })

  return { systemMessage, userMessage }
}

export function buildPrompt(config: TestConfig, promptConfig?: PromptConfig): string {
  const { systemMessage, userMessage } = buildMessages(config, promptConfig)
  return `${systemMessage}\n\n${userMessage}`
}

// 辅助函数
function getGradeName(grade: string): string {
  const gradeNames: Record<string, string> = {
    "1": "一年级",
    "2": "二年级",
    "3": "三年级",
    "4": "四年级",
    "5": "五年级",
    "6": "六年级",
  }
  return gradeNames[grade] || "小学"
}

function getDifficultyName(difficulty: string): string {
  const difficultyNames: Record<string, string> = {
    low: "基础",
    medium: "中等",
    high: "提高",
  }
  return difficultyNames[difficulty] || "标准"
}

/**
 * 防御性 JSON 解析
 * - 去除字符串外部的控制字符
 * - 最多尝试 3 层解析（处理模型返回多层字符串包裹的情况）
 */
function safeJsonParse(text: string): unknown {
  let cleaned = text
    .replace(/[\u0000-\u001F]+/g, (m) => {
      if (m === "\n" || m === "\r" || m === "\t") return ""
      return ""
    })
    .trim()

  for (let i = 0; i < 3; i++) {
    try {
      console.log(`[AI Parse Attempt ${i + 1}]`, cleaned)
      const parsed = JSON.parse(cleaned)
      if (typeof parsed === "string") {
        cleaned = parsed
        continue
      }
      return parsed
    } catch {
      break
    }
  }
  throw new Error("Failed to parse JSON from API response.")
}
